#!/usr/bin/env python3
# Taken from: https://github.com/aerissecure/rdpy
# RDPy
#
...
class RDPSocket:
    ...
    def connect(self):
        ...
        try:
            ...
            err_code = e.errno if hasattr(e, 'errno') and e.errno is not None else (e.args[0] if e.args else None)
            if err_code == errno.ECONNREFUSED or isinstance(e, socket.timeout):
...
class MCSConnectInitial:
    ...
    def __init__(self, encryption_method):
        ...
        pdu  =  b'\x03\x00\x01\xa2\x02\xf0\x80\x7f\x65\x82\x01\x96\x04\x01\x01\x04\x01'
        pdu += b'\x01\x01\x01\xff\x30\x20\x02\x02\x00\x22\x02\x02\x00\x02\x02\x02\x00'
        pdu += b'\x00\x02\x02\x00\x01\x02\x02\x00\x00\x02\x02\x00\x01\x02\x02\xff\xff'
        pdu += b'\x02\x02\x00\x02\x30\x20\x02\x02\x00\x01\x02\x02\x00\x01\x02\x02\x00'
        pdu += b'\x01\x02\x02\x00\x01\x02\x02\x00\x00\x02\x02\x00\x01\x02\x02\x04\x20'
        pdu += b'\x02\x02\x00\x02\x30\x20\x02\x02\xff\xff\x02\x02\xfc\x17\x02\x02\xff'
        pdu += b'\xff\x02\x02\x00\x01\x02\x02\x00\x00\x02\x02\x00\x01\x02\x02\xff\xff'
        pdu += b'\x02\x02\x00\x02\x04\x82\x01\x23\x00\x05\x00\x14\x7c\x00\x01\x81\x1a'
        pdu += b'\x00\x08\x00\x10\x00\x01\xc0\x00\x44\x75\x63\x61\x81\x0c\x01\xc0\xd4'
        pdu += b'\x00\x04\x00\x08\x00\x20\x03\x58\x02\x01\xca\x03\xaa\x09\x04\x00\x00'
        pdu += b'\x28\x0a\x00\x00\x68\x00\x6f\x00\x73\x00\x74\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x04\x00\x00\x00\x00\x00\x00\x00\x0c\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\xca\x01\x00\x00\x00\x00'
        pdu += b'\x00\x18\x00\x07\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        pdu += b'\x00\x00\x00\x00\x00\x04\xc0\x0c\x00\x09\x00\x00\x00\x00\x00\x00\x00'
        # insert encryption type into the PDU
        pdu += (b'\x02\xc0\x0c\x00' + struct.pack('<B', encryption_method) + b'\x00\x00\x00\x00\x00\x00\x00\x03\xc0\x20\x00\x02')
        pdu += b'\x00\x00\x00\x63\x6c\x69\x70\x72\x64\x72\x00\xc0\xa0\x00\x00\x72\x64'
        pdu += b'\x70\x64\x72\x00\x00\x00\x80\x80\x00\x00'
        self.pdu = pdu
...
class MCSConnectResponse:#response to MCS connection
    ...
    def __init__(self, resp):
        ...
        self.decoded_resp = decoder.decode(resp[7:])[1] # type: ignore
        # self.decoded_resp
        try:
            security_data = re.search(rb"\x02\x0c..(.{16})", self.decoded_resp, re.DOTALL).groups()[0] # type: ignore
...
class RDPConfig:
    ...
    def results(self, fmt=None):
        ...
        print('Port:        %s' % self.port if self.port != 3389 else 'Port:        3389 (default)')
