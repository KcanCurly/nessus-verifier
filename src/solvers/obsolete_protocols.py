    ...
    for host in hosts:
        try:
            ...
            if ip in nm.all_hosts():
                nmap_host = nm[ip]
                if  nmap_host['tcp'][7]['name'].lower() == 'echo':
                    vuln_echo.append(host)
                if  nmap_host['tcp'][9]['name'].lower() == 'discard':
                    vuln_discard.append(host)
                if  nmap_host['tcp'][13]['name'].lower() == 'daytime':
                    vuln_daytime.append(host)
                if  nmap_host['tcp'][17]['name'].lower() == 'qotd':
                    vuln_qotd.append(host)
                if  nmap_host['tcp'][19]['name'].lower() == 'chargen':
