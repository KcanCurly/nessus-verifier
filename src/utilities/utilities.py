...
def add_default_version_subservice_arguments(parser, add_target_argument = True):
    ...
    parser.add_argument("-s", "--space", type=int, default=4, help="Amount of spaces to prepend when printing affected hosts. (Default = 4)")    
...
def get_url_response(url, timeout=5, redirect = True, errors = False, verbose = False):
    try:
        ...
        if "You're speaking plain HTTP to an SSL-enabled server port" in resp.text or "HTTP request was sent to HTTPS" in resp.text: 
...
def get_latest_version(product, print_error = False):
    try:
        ...
        if print_error:
            print(f"Could not get latest version for {product}: {e}")
        return None


def get_cves2(cpe, sort_by_epss = False, limit = 10, cves_to_skip = []):
    try:
        z = nvdlib.searchCVE(cpeName = cpe, key=nvd_api_key, isVulnerable="") # type: ignore
        ...
        return [a for a in sorted_z[0:limit] if a not in cves_to_skip]
    except Exception as e:
